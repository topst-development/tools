...
TCCIMG_CERT_FORMAT = "<4s8sL4s12s4s16s12s64s16s48s"
TCCIMG_CERT_LENGTH = struct.calcsize(TCCIMG_CERT_FORMAT)
TCCIMG_CERT_IV_OFFSET = 36
TCCIMG_CERT_ENC_OFFSET = 64
TCCIMG_CERT_IV_SIZE = 16

TCCIMG_HEADER_MARKER = b"HDR\0"
TCCIMG_HEADER_FORMAT = "<4sLLL4s12s16sQL36s32sLLLLLL16s24s"
TCCIMG_HEADER_LENGTH = struct.calcsize(TCCIMG_HEADER_FORMAT)
TCCIMG_HEADER_IV_OFFSET = 152
TCCIMG_HEADER_IV_SIZE = 16
...
TCCIMG_IMAGES[b"OPTEE".ljust(12, b"\0")] = 0x3253
...
TCCIMG_IMAGES[b"FWDN-FW".ljust(12, b"\0")] = 0x3353
TCCIMG_IMAGES[b"SIC-FW".ljust(12, b"\0")] = 0x01FF
...
class signer:
    ...
    def proc_cert(self, out):
        cert_iv = get_random_bytes(16)
        self.cert[6] = cert_iv
        self.cert[8] = self.ikey.getverifykey()
        self.cert[9] = self.ikey.getenc128key()
        buf = struct.pack(TCCIMG_CERT_FORMAT, *self.cert)
        signature = tcc_sign(self.mkey.getsignkey(), buf)
        out.write(buf[:TCCIMG_CERT_ENC_OFFSET] + tcc_encrypt(self.mkey.getenc128key(), buf[TCCIMG_CERT_ENC_OFFSET:], cert_iv))
        out.write(signature)

    def proc_hdr(self, out):
        self.hdr_iv = get_random_bytes(16)
    ...
    def proc_body(self, out):
        ...
        if self.enc == True:
            out.write(tcc_encrypt(self.ikey.getenc128key(), self.img_buf, hdr_iv))
        else:
...
class verifier:
    ...
    def proc_cert(self, f):
        buf = f.read(TCCIMG_CERT_ENC_OFFSET)
        cert_iv = buf[TCCIMG_CERT_IV_OFFSET:TCCIMG_CERT_IV_OFFSET + TCCIMG_CERT_IV_SIZE]
        buf += tcc_decrypt(self.mkey.getenc128key(), f.read(TCCIMG_CERT_LENGTH - TCCIMG_CERT_ENC_OFFSET), cert_iv)
        ...
        print("  - IV          : {}".format(binascii.hexlify(self.cert[6])))
        res = True if self.cert[8] == self.ikey.getverifykey() else False
        print("  - Pub Key     : {} ({})".format(binascii.hexlify(self.cert[8]), res))
        res = True if self.cert[9] == self.ikey.getenc128key() else False
        print("  - Enc Key     : {} ({})".format(binascii.hexlify(self.cert[9]), res))
    ...
    def proc_hdr(self, f):
        ...
        if self.header[15] == 0:
            ...
            hdr_iv = buf[TCCIMG_HEADER_IV_OFFSET:TCCIMG_HEADER_IV_OFFSET + TCCIMG_HEADER_IV_SIZE]
            self.img = tcc_decrypt(self.ikey.getenc128key(), f.read(self.header[14]), hdr_iv)
        ...
        print("  - IV          : {}".format(binascii.hexlify(self.header[17])))
