...
from tccmcert import tcc_get_rpub_keytable
...
SSSHSM_RPUB_KEYTABLE_OFFSET = 0x6ff0
...
#
class signer:
    def __init__(self, src, rbid, fac, keybase=KEY_BASE):
        self.fac = fac
        self.rbid = rbid
    ...
    def write_rpub_keytable(self, hsm_image):
        keytable = tcc_get_rpub_keytable()
        hsm_image.seek(SSSHSM_RPUB_KEYTABLE_OFFSET)
        hsm_image.write(keytable)

    def process(self, name):
        with open(name, "wb") as out:
            self.proc_ssshsm(out)
        with open(name, "r+b") as hsm_image:
            self.write_rpub_keytable(hsm_image)
...
if (__name__ == "__main__"):
    ...
    if args.verify is None:
        ...
        signer(args.input, args.rbid, args.factory, args.keybase).process(args.output)
    else:
